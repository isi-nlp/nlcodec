...
NOTE: Install PySpark using `pip install pyspark`

[source,bash]
----
python -m nlcodec.learnpro
...
$ python -m nlcodec.learnpro -i train.eng.tok train.kan.tok -l bpe -vs 8000  -m ~/tmp/bpe.8k.model

# It created, two intermediate files
~/tmp/bpe.8k.charfreq.gz
~/Users/tg/tmp/bpe.8k.wordfreq.gz
----
